...
namespace AERChartPlugin\Assets;

use AERChartPlugin\Core\Template;
use AERChartPlugin\Traits\Base;
use AERChartPlugin\Libs\Assets;

/**
 * Class Frontend
 *
 * Handles frontend functionalities for the AERChartPlugin.
 *
 * @package AERChartPlugin\Assets
 */
class Frontend {
	...
	/**
	 * Script handle for AERChartPlugin.
	 */
	const HANDLE = 'wordpress-plugin-boilerplate-frontend';

	/**
	 * JS Object name for AERChartPlugin.
	 */
	const OBJ_NAME = 'wordpressPluginBoilerplateFrontend';

	/**
	 * Development script path for AERChartPlugin.
	 */
	...
	/**
	 * Check if shortcodes are present in the current post/page content.
	 *
	 * @return void
	 */
	public function check_for_shortcodes() {
		...
		// Check if shortcodes are present in the content.
		if ( has_shortcode( $post->post_content, 'aer_commodities' ) || has_shortcode( $post->post_content, 'aer_basket_prices' ) ) {
			self::mark_shortcode_present();
		}
	}

	/**
	 * Track if shortcodes are present on the page.
	 *
	 * @var bool
	 */
	...
	/**
	 * Mark that shortcodes are present and assets should be enqueued.
	 *
	 * @return void
	 */
	public static function mark_shortcode_present() {
	...
	/**
	 * Force enqueue assets (used by shortcodes).
	 *
	 * @return void
	 */
	private function force_enqueue() {
		if ( ! did_action( 'wp_enqueue_scripts' ) ) {
			return;
		}

		Assets\enqueue_asset(
			AER_DIR . '/assets/frontend/dist',
			self::DEV_SCRIPT,
			$this->get_config()
		);
	...
	public function enqueue_script( $screen ) {
		// If shortcodes are present, always enqueue.
